...
//}
port::StatusOr<std::unique_ptr<StreamExecutor>>
ExecutorPlatform::GetUncachedExecutor(
    const StreamExecutorConfig& config) {
  auto executor = absl::make_unique<StreamExecutor>(
      this, absl::make_unique<ExecutorExecutor>(config.plugin_config));
  auto init_status = executor->Init(config.ordinal, config.device_options);
  if (!init_status.ok()) {
    return port::Status{
        port::error::INTERNAL,
...
static void InitializeExecutorPlatform() {
  ...
  std::unique_ptr<Platform> platform(new ExecutorPlatform);
...
REGISTER_MODULE_INITIALIZER(
    ...
    stream_executor::executor::InitializeExecutorPlatform());
